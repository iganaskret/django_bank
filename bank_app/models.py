...
class Ledger(models.Model):
   ...
   def transaction(cls, amount, from_account, to_account, text):
        ...
        ledger = Ledger()
        ledger = cls(id_account_fk=sender_account, amount=-amount, text=text, transaction_id=id)
        ledger.save()
        receiver_account = get_object_or_404(Account, pk=to_account)
        ledger = Ledger()
        ledger = cls(id_account_fk=receiver_account, amount=amount, text=text, transaction_id=id)
        ledger.save()
